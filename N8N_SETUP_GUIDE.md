...
4. Di kanan panel, konfigurasi:
   ...
   - **URL**: Akan otomatis menjadi: `http://localhost:5678/webhook-test/Laptop-diagnose`
...
```env
VITE_N8N_WEBHOOK_URL=http://your-n8n-url:5678/webhook-test/Laptop-diagnose
```
...
VALUES ('{{ $json.wifiName }}', '{{ $json.phone }}', '{{ $json.diagnosis.nama }}', {{ $json.diagnosis.cf }}, '{{ $json.timestamp }}')
```
...
3. Konfigurasi:
   ...
   - **Body**:
     ```json
     {
       ...
       "body": {
         "params": ["{{ $json.wifiName }}", "{{ $json.diagnosis.nama }}"]
       }
     }
     ```
...
```env
# Development
VITE_N8N_WEBHOOK_URL=http://localhost:5678/webhook-test/Laptop-diagnose

# Production (contoh)
# VITE_N8N_WEBHOOK_URL=https://n8n.example.com/webhook-test/Laptop-diagnose
```
